...
const pintarCarrito = () => {
  ...
  // === Clase 12, Bloque 1: actualizar el título de la pestaña con el contador ===
  // Implementación solicitada: document.title refleja la cantidad de productos.
  document.title = cantidad === 0 ? "TechCart — Tienda de electrónica" : `TechCart — ${cantidad} productos`

  // === Clase 12, Bloque 1: habilitar/deshabilitar el botón Confirmar ===
  // El botón `#confirmar` debe activarse cuando exista al menos UN item en el carrito.
  // Usamos `some` para comprobar si hay algún item con cantidad > 0.
  if (confirmarBtn instanceof HTMLButtonElement) {
    try {
      confirmarBtn.disabled = !carrito.some(item => item.cantidad > 0)
    } catch (e) {
      console.warn('Error comprobando confirmarBtn:', e)
    }
  }
}
